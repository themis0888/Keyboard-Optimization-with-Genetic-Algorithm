...
def fitness(sol):
    ...
    fit_val += 1.0 * fitness_area(sol) + fitness_seq(sol) + fitness_finger(sol)
...
def fitness_seq(sol, scale = 1):
    ...
    key2_list = dict()
    key_list = dict()
    for i in range(sol.num_alphabet):
        key_list[CONFIG['key_string'][i]] = (finger_list[i], pos_list[i])
    ...
    for pair in seq_freq:
        ...
        # same finger but not same key - fitness falue = 10 to 20
            ...
            #print(pair, key_list[pair[0]][0], seq_freq[pair])
...
A = Solution(keyboard.alp_array)
A.which_finger = CONFIG['finger_list']
print(fitness(A))